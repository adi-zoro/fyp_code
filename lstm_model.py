...
def plot_tile_overlap(df_results, save_path="tile_overlap_plot.png"):
    """
    Plots the tile overlap ratio over frames.
...
def calculate_tile_overlap(fov_pred, fov_actual, video_width, video_height, N_x, N_y):
    """
    Calculates the tile overlap ratio between predicted and actual FOVs.
    
    Args:
        fov_pred (tuple): Predicted FOV coordinates ((top_left_x, top_left_y), (bottom_right_x, bottom_right_y)).
        fov_actual (tuple): Actual FOV coordinates ((top_left_x, top_left_y), (bottom_right_x, bottom_right_y)).
        video_width (int): Width of the video in pixels.
        video_height (int): Height of the video in pixels.
        N_x (int): Number of horizontal tiles.
        N_y (int): Number of vertical tiles.

    Returns:
        float: Tile overlap ratio (percentage of overlapping tiles).
    """
    tile_width = video_width // N_x
    tile_height = video_height // N_y
...
def process_video_tile_overlap(video_path, frame_range, predictions_df, actual_df, video_width, video_height, N_x, N_y):
    """
    Processes a range of frames, calculates tile overlap ratios, and returns a DataFrame of results.

    Args:
        video_path (str): Path to the video file.
        frame_range (list): List of frame numbers to process.
        predictions_df (pd.DataFrame): DataFrame containing predicted head/eye tracking data.
        actual_df (pd.DataFrame): DataFrame containing actual head/eye tracking data.
    ...
    Returns:
        pd.DataFrame: DataFrame with frame numbers and tile overlap ratios.
        float: Average tile overlap ratio across all processed frames.
    """
    results = []
...
### automation
# Set mixed precision policy for performance optimization
...
#video_ids = [13, 1, 8]  # Assuming video IDs are mapped accordingly
video_ids = [13]
...
# Create a copy of the frame column for tracking
        ...
        sequence_length = 6  # Length of each sequence (History) 
        n_frames_ahead = 12 # Predict 5 frames ahead (Prediction Horizon) 
        ...
        # Define Checkpoint Callback (Save every 5 epochs)
